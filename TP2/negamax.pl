...
loop_negamax(J,P,Pmax,[[Coup,Suiv]|Succ],[[Coup,Vsuiv]|Reste_Couples]) :-
	loop_negamax(J,P,Pmax,Succ,Reste_Couples), % On s'assure que tous les coups possibles soient traités.
	adversaire(J,A), % On déclare ladversaire de J  
	Pnew is P+1, % On incrémente la profondeur
	negamax(A,Suiv,Pnew,Pmax, [_,Vsuiv]). %  On assigne, comme valeur de la situation suivante, la valeur du meilleur coup possible que peut jouer l'adversaire de J dans la situation induite par le coup de J étudié selon negamax.
...
meilleur([First|Rest], Best) :-
    meilleur(Rest, Best_Rest),
    meilleur_des_deux(First, Best_Rest, Rest).

meilleur_des_deux(A, B, A) :-
    A @< B, !.
meilleur_des_deux(A, B, B). */
